...
import { STORE_QUIZ } from "@/redux/store-quiz.action"
...
export default function Results() {
  ...
  const quizName = useSelector((state: { quizName: string }) => state.quizName)
  const { email, token } = useSelector((state: { user: { email: string, token: string } }) => state.user)
  ...
  const storeQuiz = () => {
    ...
    // @ts-expect-error can not find solution
    dispatch(STORE_QUIZ(payload));
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 pb-8">
      ...
      <div className="text-center">
        ...
        <Button className="ml-5 text-base px-7 rounded cursor-pointer" onClick={storeQuiz}>
          Store Quiz
        </Button>
      </div>
    </div>
