...
export default function LandingPage() {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setFadeOut(true);
      setTimeout(() => setShowTicker(false), 1500);
    }, 2800);
  ...
  return (
    <>
      {showTicker ? (
        <div
          className={`flex justify-center items-center h-screen text-9xl font-semibold transition-opacity duration-1000 ${fadeOut ? 'opacity-0' : 'opacity-100'
            }`}
        >
          <NumberTicker value={100} startValue={0} />
        </div>
      ) : (
        <div>
